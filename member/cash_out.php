...
class ZPage extends XPage
{
    ...
    function getProcessor ($value = 0)
    {
        ...
        $allow_another_processor = $this->db->GetOne ("Select is_active From `processors` Where processor_id='13'", 0);

        //if ($allow_another_processor == 1 And $account_id != "")
         if ($account_id != "")
        {
            $selected = ($value == 13)? "selected" : "";

            $name = $this->db->GetOne ("Select name From `processors` Where processor_id='13'", "");

            $toRet .= "<option value='13' $selected>".$name."</option>";
        }
