...
//		if ($_SERVER['QUERY_STRING']=='' || isset($_GET['overview'])) {
        ...
        if ($cycling == 1) {
            if ($level == 0) {
               ...
               $f_ref= $thisSiteUrl . "?ref=".$ref_id;
                  $ref_link =  '<a href="'.$f_ref.'"/>'.$f_ref.'</a>'; //$thisSiteUrl . "?ref=" . $ref_id;
           ...
           // $ref_link = '<input type="text" class="form-control" placeholder="" value="' . $thisSiteUrl . "?ref=" . $ref_id . '" >';
              $ref_link = $thisSiteUrl . "?ref=" . $ref_id ;
...
//		if (isset($_GET['paymentsettongs'])) {
        ...
        $bank_name = "<input type='text' name='bank_name' value='$bank_name' style='width:200px;' maxlength='150'>";

        $account_name = $row['account_name'];
        $account_name = "<input type='text' name='account_name' value='$account_name' style='width:200px;' maxlength='150'>";

        $bank_address = $row['bank_address'];
       // $bank_address = "<input type='text' name='bank_address' value='$bank_address' style='width:200px;' maxlength='150'>";
        ...
        $this->data += array(
            ...
            //"ACCOUNT_BANKADDRESS" => $bank_address,
        );
//		}
// =============================
    ...
    function selectProcessor($value = 0)
    {
        ...
        $result = $this->db->ExecuteSql("Select processor_id, name From `processors` Where  processor_id=13 Order By name");
        ...
        while ($row = $this->db->FetchInArray($result)) {
           // $selected = ($row['processor_id'] == $value) ? "selected" : "";
             $selected = ($row['processor_id'] == 13) ? "selected" : "";
    ...
    function ocd_update()
    {
        ...
        if ($this->GetGP('ocd_type') == 'PAYMENTSETTONGS') {
            ...
            $bank_name = $this->enc($this->GetValidGP("bank_name", $dict['MyAcc_Bank'], VALIDATE_NOT_EMPTY));
            $account_name = $this->enc($this->GetValidGP("account_name", $dict['MyAcc_Name'], VALIDATE_NOT_EMPTY));
          //  $bank_address = $this->enc($this->GetValidGP("bank_address", $dict['MyAcc_BankAddress'], VALIDATE_NOT_EMPTY));
        }
...
//debug($this->errors);
        if ($this->errors['err_count'] > 0) {
            ...
            $this->data = array(
             ...
             "ACCOUNT_BANK" => "<input type='text' name='account_name' value='" . $account_name . "' maxlength='150' style='width: 200px;'>",
                "ACCOUNT_BANK_ERROR" => $this->GetError("account_name"),
            
             "ACCOUNT_NAME" => "<input type='text' name='bank_name' value='" . $bank_name . "' maxlength='150' style='width: 200px;'>",
                "ACCOUNT_NAME_ERROR" => $this->GetError("bank_name"),
            
            // "ACCOUNT_BANKADDRESS" => "<input type='text' name='bank_address' value='" . $bank_address . "' maxlength='150' style='width: 200px;'>",
              //  "ACCOUNT_BANKADDRESS_ERROR" => $this->GetError("bank_address"),
            ...
            if ($this->GetGP('ocd_type') == 'PAYMENTSETTONGS') {
                $this->db->ExecuteSql("Update `members` Set processor='$processor', account_id='$account_id', bank_name='$bank_name', account_name='$account_name' Where member_id='$member_id'");
