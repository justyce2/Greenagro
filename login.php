...
class ZPage extends XPage
{
    ...
    function ocd_list ($a = "")
    {
        ...
        $msss = $this->GetGP('reg');
        $displa = '';
        if($msss== 'yes') $displa = 'You have completed the first step to your registration, Kindly  Login to make payment and complete the last step'; 
    ...
    function ocd_login ()
    {
        ...
        switch ($result)
        {
            ...
            break;
            case 1:
                 $level_id = $this->db->GetOne("Select m_level From `members` Where username='$login'", 0);
                 if($level_id > 0){
                    $this->Redirect ("./member/myaccount.php");  
                 }else{
                      $this->Redirect ("./member/payment.php");
                 }
