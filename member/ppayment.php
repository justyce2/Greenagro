...
class ZPage extends XPage
{
    ...
    <div class='form-group'>
        <div class='row'>
            <label class='col-sm-4 control-label'>Select Mode of Payment :</label>
            <div class='col-sm-8'>
            ...
            " : "";
            $quant = ($this->GetGP ("quantity", "") > 0)?$this->GetGP ("quantity", ""): 1;

$quantitity =  "<input type='number' name='quantity' value=$quant min='1' max='1000'   required='yes'/>";
        ...
        $this->data = array (
            ...
            "ROW_QUANTITY" => $quantitity,
        );

            if ($prepayment == 1)
            {
                
                $amount = $product ["price"] *$quant;
    ...
    function select_processor ($processor_id = 0)
    {
        ...
        $toRet .= "<option value='0' >Choose Mode of Payment</option>";
        ...
        while ($row = $this->db->FetchInArray ($result))
        {
            ...
            $toRet .= "<option value='".$id."'>".$row['name']." (fee: $fee%)</option>";
        }
        return $toRet."</select>\r\n";
    }
