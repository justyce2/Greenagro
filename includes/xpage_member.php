...
class XPage
{
    ...
    function XPage ($object = "none", $checkAccess = true)
    {
        ...
        //if ( substr($HTTP_HOST,0,2)!=substr($license_key,0,2) || !in_array($this->lic_key, array('FREE', 'STARTER', 'STANDARD') ) )exit('Access denied');
//        if ( substr($_SERVER['HTTP_HOST'],0,2)!=substr($license_key,0,2) || !in_array($this->lic_key, array('FREE', 'STARTER', 'STANDARD') ) ) 
	...
	$overview_class = (basename ($this->pageUrl) == "overview.php")? "active" : "";
		...
		if ($cycling == 1)
        {
            ...
            $hdrMenuMatrix = "<li class='".$matrix_class."'><a href='matrix.php' >{$dict['Menu_MyMatrices']}</a></li>";
        ...
        $friend_class = (basename ($this->pageUrl) == "tellfriend.php")? "active2" : "inactive2";
        $hdrMenuFriend = "<li><a href='tellfriend.php' ><i class='fa fa-comments fa-lg'></i>{$dict['Menu_TellFriends']}</a></li>"; 
