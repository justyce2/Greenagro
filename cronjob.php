...

    {
        ...
        //schedule for payment 
...

            {
                ...
                $att = 'For completed matrix (Level: #'.$m_level.', Number: #1)'; 
                 $sum_to_cash = $this->db->GetOne("Select host_fee From `types` Where order_index='$m_level'", 0);
                $iscredited = $this->db->GetOne ("Select Count(*) From `cash` Where to_id='$id' AND type_cash='$m_level'  AND  amount='$sum_to_cash'", "0.00");
                	if($iscredited == 0){
				  	//echo $id.'/level='.$m_level.'<br>';
				   
				  }

				}
                  
                $balance = $this->db->GetOne ("Select SUM(amount) From `cash` Where to_id='$id'", "0.00");
