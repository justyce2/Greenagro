...
if ($res->status == 'successful') {
		...
		// $secretkey =   'FLWSECK_TEST-8fd3bcc7041a8842e92bccc8f8abae9c-X';
...
curl_setopt_array($curl, array(
  ...
  CURLOPT_HTTPHEADER => array(
    ...
    "Authorization: Bearer FLWSECK-674c156004df0c7bb663b5bc2f7ee905-X"
  ),
));
  ...
  $item_level = $data->meta->level_id;
    $currency = $data->currency;
            ...
            $sum = $this->db->GetOne ("Select entrance_fee From `matrixes` Where matrix_id=2");
									...
									$processor_fee = ($processor_fee/100)  * $sum;
								//	$sum = ($sum + $sum / 100) * 1.4;
									$sum = $sum + $processor_fee;
					...
					//	echo $sum.'/amoum'.$amount ;	
					//	die;
            
            if($product_type == 'Membership Fee' && $currency=='NGN'  && $amount ==$sum  && $count == 1 && $status == 'success'  && $count2==0 ){
			...
			}else if($product_type == 'Product Payment' && $currency=='NGN' && $status == 'success'){
