    ...
    function ocd_list ()
    {
        ...
        if ($total > 0)
        {
            ...
            while ($row = $this->db->FetchInArray ($result))
            {
                ...
                $account_name = $this->db->GetOne ("Select account_name From `members` Where member_id='$member_id'", "");
                 $bank_name = $this->db->GetOne ("Select bank_name From `members` Where member_id='$member_id'", "");
               ...
               // $account_name = $this->dec ($row['account_name']);
               // $bank_name = $this->dec ($row['bank_name']);
               // $bank_address = $this->dec ($row['bank_address']);
                $title_status = "&nbsp;";
                $with_fee = $amount - $fee;
                ...
                $this->data ['TABLE_ROW'][] = array (
                    ...
                    "ROW_PROCESSOR" => $account_id,
                    "ROW_ACCOUNT_NAME" => $account_name,
                    "ROW_BANK_NAME" => $bank_name,
                   // "ROW_BANKADDRESS" => $bank_address,
