...
function in_matrix($member_id, $enroller_id, $m_level)
{
    ...
    if ($top_place_id > 0) {
        ...
        if (Count($count_under_place_id) == $total_to_complete) {
           ...
           // $checkOlPlace = $db->GetOne("Select Count(*) From `places` Where `member_id`='$hoster_id' And `m_level`=5", 0);
            if($m_level == 4){
                 $entry4 = $db->GetOne("Select `reentry` From `places` Where `member_id`='$hoster_id' And `m_level`='$m_level'  Order by `place_id` LIMIT 1 Desc");
                 
                 $nextentry = $entry4 +1;
                 
                 $checkOlPlace = $db->GetOne("Select Count(*) From `places` Where `member_id`='$hoster_id' And `m_level`=1 AND reentry='$nextentry'", 0);
                 
                 if($checkOlPlace  ==0 ){
                     in_matrix($hoster_id, $enroller_id, 1);
                     $sum_minus_cash = $db->GetOne("Select cost From `types` Where order_index='1'", 0);
                     $sum = $db->GetOne ("Select entrance_fee From `matrixes` Where matrix_id=2");
                     $sum_minus_cash = $sum_minus_cash + $sum;
...
function matrix_tree_member_cycl($host_id, $m_level, $place_id, $width, $depth)
{
    ...
    if ($mm_level >= $m_level) {
        ...
        $content .= "<tr><td style='border:1px solid #cccccc;text-align:center;background-color:green;color:white'>$name (R#$reentry)<br />ID:$host_id<br />{$dict['MX_Level']}:$mm_level</td></tr><tr><td>";
        $content .= get_width_ref_member($host_id, $place_id, $width, $depth, 0);
       // $content .="<tr><td>View Tree</td></tr>";
        $date_complete = $db->GetOne("Select `z_date` From `matrices_completed` Where place_id='$place_id'", 0);
        $content .= ($date_complete > 0) ? "</td></tr><tr><td style='background-color:#9ED900; color:white'>{$dict['MX_Status']}: <b>{$dict['MX_Completed']} " . date("d M Y H:i", $date_complete) . "</b></td><tr> " : "</td></tr><tr><td style='background-color:green; color:white;'>{$dict['MX_Status']}: <b>{$dict['MX_Incompleted']}</b></span></td><tr>";
        $content .= "</table>";
...
function get_width_ref_member($host_id, $place_id, $width, $depth, $sts)
{
    ...
    if ($count_ref > 0) {
        ...
        while ($row = $db->FetchInArray($result)) {
            ...
            $lll = 'matrix.php?dlin='.$member_id;

            $to_ret .= "<td style='text-align:center;width:$percent%' border=1><div><img  src='images/unnamed.png' width='50px'/></div><div style='font-size:10px;height:30px;'> <a href=".$lll.">View Downline</a></div><div style='font-size:10px;height:80px;'>$member_info</div>";
    ...
    if ($count_ref < $width) {
        $number = $width - $count_ref;
        for ($i = 1; $i <= $number; $i += 1) {
            $to_ret .= "<td style='text-align:center;width:$percent%'><div style='height:80px;'><img src='./images/grey_man.png' style='width:35px;' border='0' alt='Vacant Place' title='Vacant Place' /></div>";
            if ($sts < $depth) {
...
function getPayFormCode($member_id, $amount, $processor_id, $product, $level_id)
{
    ...
    $currency = $sql['symbol'];
 $sql1 = $db->ExecuteSql("Select * From `members` Where member_id='$member_id'");
 if ($row1 = $db->FetchInArray($sql1)) {
         $email = $row1['email'];
    ...
    if ($row = $db->FetchInArray($result)) {
        ...
        return "<h4 style='color:red;'>Error: Please Select Mode of Payment Above</h4>";
    }
	...
	$redirect_url = $SiteUrl . "member/payment_res.php";
...
//    $description = ($product != "Product Payment")? "$SiteTitle Membership Fee" : "$SiteTitle Membership Fee";
    ...
    switch ($proc_keyname) {
	 ...
	 case "flutterwave":
            $notify = $SiteUrl . "notify/flutterwave.php";
            $code = 
				
				
				"<form method='POST' action='$routine_url'>
  <input type=\"hidden\" name=\"public_key\" value=\"$account_id\" />
  <input type=\"hidden\" name=\"customer[email]\" value=\"$email\" />
  <input type=\"hidden\" name=\"customer[phone_number]\" value=\"$phone\" />
  <input type=\"hidden\" name=\"customer[name]\" value=\"$f_name\" \"$l_name\" />
  <input type=\"hidden\" name=\"tx_ref\" value=\"$product\" />
  <input type=\"hidden\" name=\"amount\" value=\"$amount\" />
  <input type=\"hidden\" name=\"currency\" value=\"$currency_name\" />
  <input type=\"hidden\" name=\"meta[member_id]\" value=\"$member_id\" />
  <input type=\"hidden\" name=\"meta[product]\" value=\"$product\" />
  <input type=\"hidden\" name=\"meta[level_id]\" value=\"$level_id\" />
